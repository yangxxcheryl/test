...
// 2017-05-26 
// 将小车抬起延时提前
// 屏蔽液路自检过程中,清洗液定标前的if(CheckLiqDetBase())判断  即case 103 case 104 不要
// 稀释液定标第一次针位置由  -100 更改为 -100 - 100
// 第一次针下降位置由+100 更为 +50
...
// 2017-06-16
// 为了山大二院 特别定制了一个主控，测试开始之前加大了稀释液管路的热身。
//吸样完成之后灯才变成红色，同时蜂鸣器作响
//修改上電之後的Read_DiluentCalChart函數，同時增加顯示及l3 原先的程序有點小BUG，錯把l1多次利用

// 2017-06-20
// 修改取片小车抬起动作,屏蔽case 8 中的 SetCardTrolleyState(2);和case 9中的 SetCardTrolleyState(0)

// 2017-06-22
// 增加3053按键锁定和开启的LED状态切换   如果锁定前LED为红,解锁之后还是为红
// 增加进片前,判断转盘是否对齐,防止卡片
// 修改休眠方式,是否需要休眠及休眠时间完全由上位机决定,休眠命令3050
